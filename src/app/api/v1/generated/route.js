...
export async function POST(req) {
  ...
  const { trip, duration, numberOfTravelers, totalBudget, activities } = await req.json();
  ...
  activities.forEach(async (item) => {
    await prisma.activity.createMany({
      data: { day: item.day, activitiesOnDay: item.activitiesOnDay, itineraryId: saveItinerary.id },
    });
  });
